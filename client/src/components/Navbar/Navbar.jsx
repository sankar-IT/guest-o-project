...
const Navbar = React.memo(({ user = null, showUserDropdown, setShowUserDropdown, handleLogout, navigate, dropdownRef, hideCart }) => {
  const { cartItems, settings, storeStatus } = useCart();
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${isScrolled ? 'py-1 md:py-2' : 'py-1.5 md:py-3'
        }`}
    >
      <div className="max-w-7xl mx-auto px-3 md:px-6">
        <div
          className={`flex items-center justify-between px-3 md:px-6 py-2 rounded-[2rem] transition-all duration-500 ${isScrolled
        ...
        >
          ...
          <nav className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <button
                ...
                className={`relative px-6 py-2 text-[11px] font-black tracking-[0.1em] uppercase transition-all duration-300 group ${isScrolled ? 'text-text-primary' : (isDarkMode ? 'text-white' : 'text-white')
                  }`}
              >
                {link.name}
                <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-0 h-1 rounded-full transition-all duration-300 group-hover:w-4 ${isScrolled ? 'bg-[#B91C1C]' : 'bg-white'
                  }`}></span>
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-1.5 md:gap-3">
            <button
              onClick={toggleTheme}
              className={`p-2.5 rounded-full border transition-all duration-500 active:scale-90 group ${isScrolled
            ...
            {!hideCart && (
              <div
                className={`relative p-2.5 rounded-full border transition-all duration-500 cursor-pointer group active:scale-90 hidden lg:flex ${isScrolled
              ...
              >
                ...
                {cartItems.length > 0 && (
                  <span className={`absolute -top-1.5 -right-1.5 w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-black border-2 animate-pulse ${isScrolled ? 'bg-[#DA9133] border-white text-white' : 'bg-white border-[#B91C1C] text-[#B91C1C]'
                    }`}>
                    {cartItems.length}
                  </span>
                )}
            ...
            {localUser && localUser.name ? (
              <div className="relative" ref={dropdownRef}>
                <div
                  ...
                  className={`flex items-center gap-2 p-1 md:pr-4 rounded-full border transition-all duration-500 cursor-pointer ${isScrolled
                ...
                >
                  ...
                  <span className={`text-[10px] font-black tracking-widest hidden md:block ${isScrolled ? 'text-text-primary' : 'text-white'
                    }`}>
                ...
                {/* Dropdown */}
                <div className={`absolute top-full right-0 mt-4 w-72 bg-background-card rounded-[2.5rem] shadow-[0_30px_100px_rgba(0,0,0,0.15)] border border-border transition-all duration-500 ${showUserDropdown ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-4'
                  }`}>
              ...
              <button
                ...
                className={`px-4 md:px-8 py-2 md:py-3 rounded-full font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] transition-all duration-300 active:scale-95 shadow-xl ${isScrolled
            ...
            <button
              ...
              className={`lg:hidden p-2 rounded-xl transition-colors ${isScrolled ? 'text-text-primary bg-background-muted' : 'text-white bg-white/10'
                }`}
            >
              <Menu size={24} />
            </button>
